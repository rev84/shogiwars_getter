...
Board = class Board {
  ...
  parse() {
    var dump, name, ref, reg, results1, teban, tebans;
    dump = this.boardDump();
    ref = window.SCENES;
    results1 = [];
    for (name in ref) {
      ...
      results1.push((function() {
        var j, results2;
        results2 = [];
        for (teban = j = 0; j <= 1; teban = ++j) {
          results2.push((function() {
            var l, len, ref1, results3;
            ref1 = tebans[teban];
            results3 = [];
            for (l = 0, len = ref1.length; l < len; l++) {
              reg = ref1[l];
              if (dump.match(new RegExp('^' + reg + '$', 'g')) && !(Utl.inArray(name, this.tags[teban]))) {
                results3.push(this.tags[teban].push(name));
              } else {
  ...
  initBoard() {
    var j, l, x;
    this.tags = [[], []];
    this.board = Utl.array2dFill(9, 9, '.');
    for (x = j = 0; j < 9; x = ++j) {
      // 歩
      this.board[x][6] = this.koma2Reg(true, 'FU');
    }
    for (x = l = 0; l < 9; x = ++l) {
      this.board[x][2] = this.koma2Reg(false, 'FU');
    }
    // 香
    this.board[0][8] = this.board[8][8] = this.koma2Reg(true, 'KY');
    this.board[0][0] = this.board[8][0] = this.koma2Reg(false, 'KY');
    // 桂
    this.board[1][8] = this.board[7][8] = this.koma2Reg(true, 'KE');
    this.board[1][0] = this.board[7][0] = this.koma2Reg(false, 'KE');
    // 銀
    this.board[2][8] = this.board[6][8] = this.koma2Reg(true, 'GI');
    this.board[2][0] = this.board[6][0] = this.koma2Reg(false, 'GI');
    // 金
    this.board[3][8] = this.board[5][8] = this.koma2Reg(true, 'KI');
    this.board[3][0] = this.board[5][0] = this.koma2Reg(false, 'KI');
    // 王
    this.board[4][8] = this.koma2Reg(true, 'OU');
    this.board[4][0] = this.koma2Reg(false, 'OU');
    // 角
    this.board[7][7] = this.koma2Reg(true, 'KA');
    this.board[1][1] = this.koma2Reg(false, 'KA');
    // 飛
    this.board[1][7] = this.koma2Reg(true, 'HI');
    return this.board[7][1] = this.koma2Reg(false, 'HI');
  }

  move(te) {
    var isSente, koma, komaFromX, komaFromY, komaToX, komaToY;
    if (!te.match(/^[\+\-]\d{4}[A-Z]{2}$/)) {
      // 指し手のフォーマットじゃない
      return false;
    }
    isSente = te.substr(0, 1) === '+';
    ...
    koma = te.substr(5, 2);
    // 移動先を上書き
    this.board[komaToX - 1][komaToY - 1] = this.koma2Reg(isSente, koma);
    // 持ち駒から打ってない場合は元を消す
    if (komaFromX !== 0) {
      this.board[komaFromX - 1][komaFromY - 1] = '.';
    }
    return true;
  }
  ...
  koma2Reg(isSente, koma) {
    var defines;
    defines = this.defineKoma2reg();
    if (isSente) {
      return defines[0][koma];
    } else {
      return defines[1][koma];
    }
  }
...
window.draw = async function() {
  var dt, game_type, game_type_class, index, is_first, is_friend, is_win, j, l, len, len1, len2, my_name, my_rank, o, op_name, op_rank, ref, ref1, res, results, results1, t, tags, tbody, td, tr, tr2, url;
  ...
  for (j = 0, len = results.length; j < len; j++) {
    ...
    tr = $('<tr>').append($('<td>').addClass(is_win === 1 ? 'win' : is_win === 0 ? 'lose' : 'draw').html(my_name)).append($('<td>').addClass('center').html(my_rank)).append($('<td>').attr('rowspan', 2).addClass(is_first ? 'sente' : 'gote').html(is_first ? '先' : '')).append($('<td>').attr('rowspan', 2).addClass(is_first ? 'gote' : 'sente').html(is_first ? '' : '先')).append($('<td>').addClass('center').html(op_rank + (is_friend ? '<br><span class="label label-danger">友達</span>' : ''))).append($('<td>').addClass(is_win === 1 ? 'lose' : is_win === 0 ? 'win' : 'draw').html(op_name)).append($('<td>').attr('rowspan', 2).addClass(game_type_class).html(game_type)).append($('<td>').attr('rowspan', 2).addClass('center').html(dt)).append($('<td>').attr('rowspan', 2).addClass('center').append($('<button>').addClass('btn btn-sm btn-primary').attr('dt-key', res.kifu_name).html('コピー').on('click', async function() {
      var rec;
    ...
    results1.push(tbody.append(tr2));
  }
  return results1;
};
