...
window.getKifuCall = function(url = null) {
  if (url !== null) {
    window.url = url;
  }
...
window.sw2csa = function(sw) {
  var buf, isFirst, j, len, name, rest, restTime, restTimes, s, te;
  buf = '';
  // バージョン
  buf += 'V2.2' + "\n";
  // 対局者名
...
window.getIndexCallbackSuccess = function(response) {
  var a, content, div, doc, isFirst, isFriend, isMe, isNext, isWin, j, l, len, len1, len2, len3, name, o, parser, player, q, rank, ref, ref1, ref2, ref3, result, url;
  // 取得制限
...
window.Utl = (function() {
  ...
  Utl.IndexedDB = (function() {
    class IndexedDB {
      ...
      // awaitで使う
      async set(key, value) {
        var putRequest, store, token, transaction;
    ...
    // タイムアウトにするミリ秒
    IndexedDB.prototype.TIMEOUT_MSEC = 1000;
